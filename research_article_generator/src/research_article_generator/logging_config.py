...
if TYPE_CHECKING:
    from .models import PlanReviewResult, StructurePlan
...
class PipelineCallbacks(Protocol):
    ...
    def on_plan_review(self, plan: StructurePlan) -> PlanReviewResult: ...
...
class RichCallbacks:
    ...
    def on_plan_review(self, plan: StructurePlan) -> PlanReviewResult:
        ...
        table = Table(title="Structure Plan", show_lines=True)
        ...
        if plan.page_budget:
            console.print(f"  Page budget: {plan.page_budget} ({plan.budget_status})")

        if plan.plan_review_notes:
            console.print()
            console.print("[bold yellow]PlanReviewer Notes:[/]")
            for note in plan.plan_review_notes.split(";"):
